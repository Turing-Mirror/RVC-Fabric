...
if torch.cuda.is_available() or ngpu != 0:
    for i in range(ngpu):
        ...
        # Prefer capability check over brittle name substrings (missed 50-series etc.)
        try:
            major, _minor = torch.cuda.get_device_capability(i)
            # CUDA compute capability >= 3.5 is usable for modern PyTorch training
            name_ok = major >= 3
        except Exception:
            name_ok = False
        if not name_ok:
            # Fallback: legacy name heuristics for odd drivers
            name_ok = any(
        ...
        if name_ok:
            # A10#A100#V100#A40#P40#M40#K80#A4500#RTX50xx
...
def change_info_(ckpt_path):
    ...
    try:
        ...
        with open(
            ...
            # train.log first line embeds a dict; never use eval() on file content
            raw = f.read().strip("\n").split("\n")[0].split("\t")[-1]
