...
export const TIPS: Record<string, string> = {
  ...
  sg_output_device:
    "变声后的声音送到哪里。\n想让游戏 / 语音 里的人听到，选 CABLE Input，然后在游戏里把麦克风选成 CABLE Output。",
  monitor_self:
  ...
  pitch:
    "音高。男声变女声通常 +12 左右，女声变男声 −12 左右。变声中可实时调整试听效果。",
  formant:
    "共鸣（共振峰）。微调音色的粗细，配合音高一起找像这个音色（角色）的位置。",
  index_rate:
  ...
  cuda_graph:
    "把每一块的推理过程录制下来重放，省掉反复下发指令的开销，能降低延迟、减少显卡占用。仅 NVIDIA 显卡有效。不生效时会自动退回原来的算法。",
  I_noise_reduce: "输入降噪。更干净，代价是更高的延迟。",
  O_noise_reduce: "输出降噪。对变声后的声音再降一次噪，一般不需要。",
  use_pv: "相位声码器。某些音色上衔接更自然，可以开着试听对比。",
  fx_enabled:
    "后期处理总开关。变声之后再过一遍噪声门、压缩器、均衡器。\n关掉就是原始输出，下面的设置全部不生效。运行中可实时开关。",
  fx_eq_enabled: "均衡器开关。不确定怎么调就先用下面的预设。",
  fx_eq_preset:
    "均衡预设。选一个直接套用，也可以选完再拖下面的推子微调。\n手动拖过之后会显示「自定义」。",
  fx_gate_enabled: "噪声门。说话间隙自动压掉底噪，比响应阈值更柔和。",
  fx_gate_threshold_db:
    "噪声门阈值。比这个还轻的声音会被压低。\n环境吵就往上调（往 −20 走），但调太高会把气音也吃掉。",
  fx_comp_enabled: "压缩器。把忽大忽小的音量压平，听感更稳。",
  fx_comp_threshold_db: "压缩阈值。超过这个响度才开始压。越低压得越多。",
  fx_comp_ratio:
    "压缩比。4:1 是常用值。比例越大越「平」，太大听起来会发闷、没有起伏。",
  fx_out_gain_db: "输出增益。压缩之后整体音量偏小的话在这里补回来。",
  ...
  telemetry_opt_in:
    "匿名用户数量统计。只发送一个随机编号、软件版本、显卡加速方式，\n不发送账号、音色、录音或任何能定位到你的信息。",
};
