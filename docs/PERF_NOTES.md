...
>
> 原 `PLAN_2026-07-20_UI与推理优化.md` 中**未做完的推理/验收路线**已收敛到本文 §「后续路线」；
> UI 已落地项见 `CONTEXT_HANDOFF.md` / `UI-AESTHETIC-DESIGN.md`，勿再开带日期的计划散页。
...
原则：**不破坏已训模型**（`.pth` / `.index` 语义不变）；改推理侧优先。

| 优先 | 项 | 说明 |
|------|----|------|
| 高 | 三变体实机验收 | nvidia / amd / nvidia50：启停、切音色、热更、监听、快捷键、强杀恢复 |
| 中 | DML/ONNX 补偿 | A/I 卡 fp32 慢于 CUDA；可选 ONNX Runtime 路径（不改原模型文件） |
| 中 | 静音块 / 默认阈值等体验向 | 多已部分落地；继续以 bench + 听感为准，勿为快而糊 |
| 低 | `torch.compile` / CUDA Graphs | 见上文 §3：投入产出比低，默认跳过 |
| 低 | exe 代码签名 / CI 跑 tests | 产品化；与推理优化独立 |

**已取消/勿回退**：f0 与 HuBERT 双流并行（f0 仅约 4ms，无肉可割）。
