# RVC Fabric — UI / 美学设计手册

> **读者说明：** 本文档描述 **RVC Fabric 产品壳**（`launcher/`）的设计原则。  
> **以代码为准：** 桌面壳色板与控件实现见 `launcher/theme.py`、`launcher/ui/`。  
> 历史名称「白无垢」仍可指气质（安静、反 AI 味）；**现行桌面色板已演化**，不再强制 accent = ink。  
> **产品显示名：** `RVC Fabric`（见 `theme.APP_PRODUCT_NAME`）。
...
命名约定：代码里 token 前缀仍为 `tm-` / `TM_`（历史标识，与产品显示名 RVC Fabric 并存）。
...
RVC Fabric 桌面壳采用 Schale 浅蓝库感 + 安静工具气质：**明确拒绝**常见 AI 产品的蓝紫渐变、青绿霓虹与 RVCMAX 粉紫壳。后续一切 UI 默认继承本文 §0，不得自行换肤。
