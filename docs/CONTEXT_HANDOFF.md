...
- **自定义背景图（2026-07-28）**：设置 →「外观（背景图）」可选本地图；**不透明度** 0–100（图案相对默认画布强度）、**磨砂** 0–40（`PIL.ImageFilter.GaussianBlur`，与 SoftSlider 阴影同源技术）；图按 CSS cover 缩放后 `Image.blend` 到 Schale 画布色。文件落 `User_Data/wallpaper/`。内容区 `TM_BG` 在 Windows 上用分层窗口色键透出壁纸（`pywinstyles` 或同款 Win32 API），导航/底栏/卡片保持实色可读。实现：`launcher/ui/wallpaper.py`。
