# -*- coding: utf-8 -*-
"""In-app usage guide content (product-specific wording)."""

from __future__ import annotations

# Sections: (eyebrow unused, title, body with optional **bold**)
HELP_SECTIONS: list[tuple[str, str, str]] = [
    (
        "",
        "这是什么软件",
        "RVC Fabric 是给游戏 / QQ / Discord 等场景用的**本地实时变声**工具。\n"
        "你对着麦克风说话，软件把声音换成所选「音色」，再通过虚拟声卡送进游戏或语音软件。\n"
        "\n"
        "日常只需要：\n"
        "1. 启动器（首次：快捷方式、虚拟声卡、环境）\n"
        "2. 主界面：选音色 → 设设备 → 点「开启变声」\n"
        "\n"
        "训练自己的音色、翻唱歌曲属于进阶，在「其他」里打开训练/翻唱 WebUI，**不是每次开黑都要开**。",
    ),
    (
        "",
        "第一次怎么开",
        "1. 解压到**尽量是英文路径**的文件夹（路径里少用生僻字，可减少运行库报错）。\n"
        "2. 双击 **启动器**。\n"
        "3. 点「发送快捷方式」——以后从桌面进主界面。\n"
        "4. 点「安装虚拟声卡」——装 **VB-Cable**（开黑必备）。\n"
        "5. 点「补全运行环境」——选择显卡分版后下载 Runtime。\n"
        "6. 「安装虚拟声卡」：软件须先完整解压到硬盘；VBCABLE 内含 Setup 与驱动，\n"
        "   会弹出 UAC，请允许管理员安装（勿从压缩包窗口直接运行）。\n"
        "6. 点「打开主界面」。\n"
        "\n"
        "请用桌面快捷方式或启动器启动。",
    ),
    (
        "",
        "声卡怎么接（最重要）",
        "请记住三条线：\n"
        "\n"
        "【在本软件「设置」里】\n"
        "· **输入设备** = 你的真实麦克风（不要选 CABLE）\n"
        "· **输出设备** = **CABLE Input**（有的列表写成 VB-Audio Virtual Cable）\n"
        "· **监听设备**（可选）= 你的耳机 / 音箱，用来「变声时监听自己」\n"
        "\n"
        "【在游戏 / QQ / Discord 里】\n"
        "· 麦克风 / 输入 = **CABLE Output**\n"
        "  这样对面听到的是变声后的声音\n"
        "\n"
        "【Windows 系统】\n"
        "· 默认播放设备 = 耳机（不要设成 CABLE，否则容易啸叫或听不到系统声音）\n"
        "\n"
        "可用启动器「系统快捷 → 声音设备」对照检查。\n"
        "用 USB 直播声卡 / 调音台的用户，见设置页「实体声卡连接说明」。\n"
        "装完 VB-Cable 后若列表没有设备：点设置里的「重载设备列表」，或重启一次软件。",
    ),
    (
        "",
        "日常开黑五步",
        "1. **首页**或**模型**页选好音色（首页可左右切换；底栏也会显示当前音色）。\n"
        "2. **设置**里确认输入/输出设备（见上一节）。需要听自己时勾选「变声时监听自己」。\n"
        "3. 点底栏 **「开启变声」**。首次加载约 20～40 秒，右下角会显示「变声中」。\n"
        "4. 游戏麦克风选 CABLE Output，正常说话即可。\n"
        "5. 结束时再点一次（变为停止），或「其他 → 强制结束变声引擎」。\n"
        "\n"
        "底栏还有 **输出变声 / 原声旁路**：\n"
        "· 输出变声 = 正常换音色\n"
        "· 原声旁路 = 不换音色，只走线路（测麦、测连接）\n"
        "运行中可切换。音高 / 共鸣 / 阈值可在底栏快速拧，并会按当前音色记住。",
    ),
    (
        "",
        "每个页面做什么",
        "· **首页**：选音色、看当前音色、开启变声。\n"
        "· **模型**：社区音色、导入 .pth/.index/.zip、检索库与配置档案绑定。\n"
        "· **设置**：设备、变声参数、性能、声音效果、快捷键、在线更新。\n"
        "· **说明**：使用教程。\n"
        "· **其他**：优化服务、诊断包、训练 WebUI、原版实时面板、强制结束引擎等。",
    ),
    (
        "",
        "音色从哪来",
        "· 「模型 → 导入音色」支持 **.pth**、**.index**、**.zip 音色包**（与 CNB 包相同结构）。\n"
        "· 或点「模型 → 社区音色」在线获取（图灵镜源 + 第三方公开模型）。\n"
        "· 固定位置：每个音色一个文件夹 `User_Data/models/<名称>/`，\n"
        "  **.pth 与 .index 放在同一文件夹**（软件会自动复制进去，不要只扔在别处）。\n"
        "· **.index** 是特征检索文件，不是训练底模；没有也能变声。\n"
        "· 每个音色可以单独记住音高、共鸣等。\n"
        "\n"
        "自己训练：用「其他 → 训练/翻唱 WebUI」。",
    ),
    (
        "",
        "设置项一句话（详细见设置页旁注释）",
        "【设备】\n"
        "· 加速后端：auto / cuda / dml / cpu——按你下载的发行包来，别混 N 卡与 A 卡 Runtime。\n"
        "· 设备类型：多数情况用 MME 最省事。\n"
        "· 输入 / 输出 / 监听：见「声卡怎么接」。\n"
        "· WASAPI 独占：容易抢设备，一般不要勾。\n"
        "\n"
        "【变声参数 · 运行中多半可热更新】\n"
        "· 响应阈值：越小越容易触发变声；环境吵可调高。\n"
        "· 音高：半音。男变女常试 +8～+12，女变男试 −8～−12。\n"
        "· 共鸣：音色明暗/腔体感，轻微拧即可。\n"
        "· Index Rate：有 .index 时才有意义。\n"
        "· 响度因子：输出响度与原声包络的混合程度。\n"
        "· 音高算法：fcpe 较均衡；rmvpe 常更稳。\n"
        "\n"
        "【性能 · 改后需重新开启变声】\n"
        "· 延迟预设：低延迟 / 均衡 / 稳定。\n"
        "· 采样长度：越大越稳、延迟越高；卡顿可略加大。\n"
        "· 输入/输出降噪：更干净但更吃显卡。\n"
        "\n"
        "【声音效果】\n"
        "· 在变声之后的噪声门 / 压缩 / EQ，默认关。",
    ),
    (
        "",
        "常见问题",
        "Q：对面听不到 / 没变声？\n"
        "A：检查游戏麦是否为 CABLE Output；本软件输出是否为 CABLE Input；是否已「开启变声」且模式为「输出变声」。\n"
        "\n"
        "Q：有变声但自己听不到？\n"
        "A：勾选「变声时监听自己」，监听设备选真实耳机，不要选 CABLE。\n"
        "\n"
        "Q：声音一卡一卡、延迟很大？\n"
        "A：略增大「采样长度」；关掉一项降噪；换 fcpe；关无关占麦软件；确认用的是对应显卡发行包。\n"
        "\n"
        "Q：停不干净、声卡一直被占？\n"
        "A：「其他 → 强制结束变声引擎」，再重开软件。\n"
        "\n"
        "Q：第一次很慢？\n"
        "A：正常，模型与 Runtime 冷启动需要时间。\n"
        "\n"
        "Q：中文路径报错？\n"
        "A：尽量把整个软件放到英文路径再试。",
    ),
    (
        "",
        "快捷键（可在设置里改）",
        "默认常见键位（以你「设置 → 快捷键」里显示的为准）：\n"
        "· 左右方向键：切换音色\n"
        "· F5：开启 / 停止变声\n"
        "· Ctrl+↑ / Ctrl+↓：音高加减\n"
        "\n"
        "游戏里也要用快捷键时，在设置中打开**全局快捷键**。",
    ),
    (
        "",
        "使用注意",
        "· 请遵守游戏与平台规则，勿用于诈骗或骚扰。\n"
        "· 本软件在本地运行，变声质量取决于音色模型与设备性能。\n"
        "· 完整包与模型请从你信任的渠道获取。",
    ),
]


def strip_md_emphasis(text: str) -> str:
    """Remove lightweight Markdown markers used in help bodies (**bold**)."""
    import re

    if not text:
        return ""
    out = re.sub(r"\*\*(.+?)\*\*", r"\1", text, flags=re.DOTALL)
    out = out.replace("**", "")
    return out


def iter_md_segments(text: str) -> list[tuple[str, str]]:
    """Split into (kind, text) where kind is 'normal' | 'bold' for **...** only."""
    import re

    if not text:
        return []
    parts: list[tuple[str, str]] = []
    pos = 0
    for m in re.finditer(r"\*\*(.+?)\*\*", text, flags=re.DOTALL):
        if m.start() > pos:
            parts.append(("normal", text[pos : m.start()]))
        parts.append(("bold", m.group(1)))
        pos = m.end()
    if pos < len(text):
        parts.append(("normal", text[pos:]))
    return parts or [("normal", text)]


def help_plain_text() -> str:
    """Flatten for dialogs (no raw markdown asterisks)."""
    parts: list[str] = ["RVC Fabric · 使用说明", ""]
    for _eye, title, body in HELP_SECTIONS:
        parts.append(f"【{title}】")
        parts.append(strip_md_emphasis(body))
        parts.append("")
    return "\n".join(parts).strip() + "\n"


# Tips for settings page: key -> short tip (shown under control + HoverTip)
SETTING_TIPS: dict[str, str] = {
    "accel": (
        "按发行包选加速方式：N 卡包用 cuda，A/I 卡包用 dml。"
        "auto 会自动选。改完需停变声再开，勿混用不同显卡的 Runtime。"
    ),
    "hostapi": (
        "音频驱动类型。Windows 上多数人用 MME 最省事；"
        "若设备异常可换其他类型后点「重载设备列表」。"
    ),
    "input": "真实麦克风。不要选 CABLE Output/Input。",
    "in_gain": (
        "麦克风前置增益（dB）。麦克风声音太小/太大时在这里补。"
        "作用于静音门与电平表之前，变声中可实时调整；"
        "加太多会削波破音，建议先试 +6 以内。"
    ),
    "output": (
        "变声结果送出的设备。开黑请选 CABLE Input，"
        "游戏里再把麦克风设为 CABLE Output。"
    ),
    "monitor_on": (
        "一边变声一边用耳机听自己。输出仍给 CABLE；" "监听请选真实耳机，不要选 CABLE。"
    ),
    "monitor": "仅用于「听自己」的耳机/音箱，不是给对面听的线路。",
    "wasapi": "独占模式可能抢走别的软件声卡。一般保持不勾选。",
    "sr_type": (
        "sr_model=按模型采样率；sr_device=按声卡采样率。" "多数情况保持默认即可。"
    ),
    "threhold": (
        "多小声才开始变声。默认约 -48 会在安静时关掉输入，减少底噪。"
        "数值越低越灵敏；环境吵时可调到 -40 左右。调到 -60 等于关闭门限。"
    ),
    "pitch": (
        "音高偏移（半音）。男声变女声常试 +8～+12；"
        "女声变男声试 −8～−12。可运行中调节，并按音色保存。"
    ),
    "formant": (
        "共鸣/音色明暗。略调可改变「腔体」感；" "拧太狠会不自然，建议小幅调整。"
    ),
    "index_rate": (
        "特征检索强度。需要已绑定 .index。"
        "0=几乎不用索引；升高更贴训练音色，也可能更耗性能或发闷。无 index 时自动当 0。"
    ),
    "rms": (
        "响度混合：默认约 0.25，小声说话时输出也偏小、大声时偏大更自然。"
        "靠近 1 更跟原声起伏；靠近 0 输出响度更「平」。"
    ),
    "f0": (
        "音高检测算法。fcpe 较均衡；rmvpe 常更稳；" "卡顿可试更轻的算法。运行中可切换。"
    ),
    "function": ("输出变声=正常换音色；输入监听=原声旁路，只测麦和连接。"),
    "index": (
        "可选的 .index 检索库，绑定到当前音色。"
        "改路径后需重新「开启变声」。不是训练用的底模。"
    ),
    "block": (
        "每次处理的音频块长度（秒）。越大越稳、延迟越高；"
        "卡顿可略加大。也可用上方「延迟预设」一键设置。改后需重新开启变声。"
    ),
    "crossfade": (
        "块与块之间的交叉淡化，减少爆音。"
        "过小可能「啪啪」响，过大略增延迟。改后需重开变声。"
    ),
    "extra": (
        "额外送进模型的上下文长度，影响稳定度与延迟。" "默认一般够用。改后需重开变声。"
    ),
    "n_cpu": (
        "部分算法（如 harvest）使用的 CPU 进程数。"
        "仅当音高算法用到时有效；不是越大越好。"
    ),
    "i_nr": "进模型前降噪。更干净，但更吃性能；小显卡可关。",
    "o_nr": "变声结果后再降噪。同样更吃性能，可与输入降噪二选一试。",
    "use_pv": "相位声码器，部分场景更平滑，也可能略改音色。按听感开关。",
    "fx_en": "总开关：变声之后的噪声门/压缩/EQ。默认关，不影响原听感。",
    "fx_gate": "小声时压低底噪。门限过低可能吃字，过高仍有噪音。",
    # 噪声门细调
    "fx_gate_thr": (
        "噪声门开合的音量线：低于它的声音会被压低。"
        "越靠近 0（如 -30）关得越勤、易切掉气声与尾音；越低（如 -70）只挡极小底噪。"
    ),
    "fx_gate_rel": (
        "声音低于门限后，多快淡到压低状态。"
        "太短会「咔哒」断音，太长则拖着底噪。常用 30～100ms。"
    ),
    "fx_gate_hold": (
        "低于门限后先保持开启的时间，避免字与字之间频繁开合。"
        "说话断断续续、被误切时可加大。"
    ),
    "fx_gate_range": (
        "门关闭时把声音压低多少。越大越安静，但开合切换越明显；" "20dB 左右通常够用。"
    ),
    "fx_comp": "压动态，让小声更大、大声不爆，开黑时声音更稳。",
    # 压缩器细调
    "fx_comp_thr": (
        "超过此电平才开始压缩。越低（如 -30）压得越多、整体更「平」；"
        "接近 0 则只压最大声的部分。"
    ),
    "fx_comp_ratio": (
        "超过阈值后的压缩强度。4:1 较通用；" "越大越像限制器，声音更稳但容易失去起伏。"
    ),
    "fx_comp_att": (
        "超过阈值后多快开始压。小值抓瞬态、更紧实；" "大值保留咬字的冲击力。"
    ),
    "fx_comp_rel": (
        "松开压缩、回到原状的速度。太短有「抽气」感，"
        "太长会压得发闷。常用 80～150ms。"
    ),
    "fx_comp_mu": (
        "压缩后补回的整体音量（makeup gain）。" "用来把压小的响度提回来，过大易破音。"
    ),
    "fx_eq": "分段均衡。可用预设「人声前倾/温暖」等，再微调各频段。",
    # 均衡 EQ 各频段（-12～+12 dB）
    "fx_eq_60": (
        "60Hz 低频/胸腔厚度。加一点更饱满；"
        "过多发闷、易喷麦。声音单薄可略加，浑浊可略减。"
    ),
    "fx_eq_250": (
        "250Hz 中低频/温暖度。过多显浑浊、有「箱音」；" "适当减少更清爽干净。"
    ),
    "fx_eq_1k": ("1kHz 中频/人声主体与存在感。加强更「靠前」；" "过多则发尖、发硬。"),
    "fx_eq_4k": ("4kHz 中高频/清晰度与咬字。加一点更清楚；" "过多齿音刺耳。"),
    "fx_eq_8k": (
        "8kHz 高频/空气感与亮度。加一点更通透；" "过多发「沙」，并放大齿音与噪声。"
    ),
    "fx_out": "效果链最后的音量加减。过大易破音。",
}
